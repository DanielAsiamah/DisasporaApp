...
Public Firebase client configuration has been filtered into the ignored local `.env` from `/Users/danielblackman/Downloads/env-backup-2026-08-31.zip`, and the user-provided Google web OAuth client ID has been added. The import intentionally excluded old-machine ElevenLabs/private values. The Google iOS client ID still needs to be retrieved from Google/Firebase Console before `npm run env:check` can pass. Store any server-side service-account file outside Git and only recreate it if a development script genuinely needs it.
...
- The Google iOS client value still needs to replace its `.env` placeholder before the full auth/device gate can pass. Firebase public client values and the Google web client value have been imported locally without copying old-machine ElevenLabs/private values.
